    ...
    try {
        ...
        const stats = await pool.query(`
            SELECT
                ...
                COUNT(*) FILTER (WHERE estado = 'verificado') AS verificados,
...
router.post('/admin/pagos/:id/verificar', verificarSesion, async (req, res) => {
    ...
    try {
        ...
        const pago = await client.query(
            `UPDATE pagos SET estado = 'verificado', fecha_verificacion = NOW(), verificado_por = $1
             WHERE id_pago = $2 AND estado = 'pendiente' RETURNING *`,
            [req.session.usuario.id_usuario, id]
        );
...
router.post('/admin/pagos/:id/rechazar', verificarSesion, async (req, res) => {
    ...
    try {
        ...
        const pago = await client.query(
            `UPDATE pagos SET estado = 'rechazado', motivo_rechazo = $1, fecha_verificacion = NOW()
             WHERE id_pago = $2 AND estado = 'pendiente' RETURNING *`,
            [motivo, id]
        );
        if (!pago.rows.length) throw new Error('Pago no encontrado o ya procesado');
        ...
        await client.query(
            `UPDATE pedidos SET estado = 'pendiente_pago' WHERE id_pedido = $1`,
            [pago.rows[0].id_pedido]
        );
...
router.post('/admin/ventas/generar', verificarSesion, async (req, res) => {
    ...
    try {
        ...
        const ultimo = await client.query(
            `SELECT numero FROM ventas WHERE numero LIKE $1 ORDER BY id_venta DESC LIMIT 1`,
            [`${prefijo}-%`]
        );
        let secuencia = 1;
        if (ultimo.rows.length) {
            secuencia = parseInt(ultimo.rows[0].numero.split('-')[1]) + 1;
        }
        const numero = `${prefijo}-${String(secuencia).padStart(6, '0')}`;

        const venta = await client.query(`
            INSERT INTO ventas (id_pedido, id_pago, numero, tipo_comprobante, total, creado_por, fecha_venta)
            VALUES ($1, $2, $3, $4, (SELECT total FROM pedidos WHERE id_pedido = $1), $5, NOW())
            RETURNING *`,
            [id_pedido, id_pago, numero, tipo_comprobante, req.session.usuario.id_usuario]
        );
